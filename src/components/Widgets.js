...
import { Col, Row, Card, Image, Button, ListGroup, ProgressBar, OverlayTrigger, Tooltip } from '@themesberg/react-bootstrap';
...
export const CounterWidget = (props) => {
  const { icon, iconColor, 
    ...
    pendingCronLogs,
    totalAmount,
    title, period, percentage } = props;
  const percentageIcon = percentage < 0 ? faAngleDown : faAngleUp;
  const percentageColor = percentage < 0 ? "text-danger" : "text-success";
  let bgClass = '';
  if(transactions > 0 && batchesNotProcessed > 0){
    bgClass = 'danger';
  } else if((transactions > 0 && batchesNotProcessed == 0) || (transactions == 0 && batchesNotProcessed > 0)){
    bgClass = 'warning';
  }
  return (
    <Card border="light" bg={bgClass} className="shadow-sm">
      <Card.Body>
            ...
            <div className="d-none d-sm-block">
              ...
              <h5>
                <OverlayTrigger
                  trigger={['hover', 'focus']}
                  overlay={
                    <Tooltip>(Not Voided but not processed)</Tooltip>
                  }
                >
                  <span>{`Transactions: ${transactions}`}</span>
                </OverlayTrigger>
              </h5>
              <h5>{`Batches (Not processed): ${batchesNotProcessed}`}</h5>
              <h5>{updatedAchFiles}</h5>
              <h5>{pendingCronLogs}</h5>
            </div>
